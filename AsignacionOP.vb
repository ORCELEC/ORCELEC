...
Public Class AsignacionOP
    ...
    Private Sub LlenaProgramaProduccion()
        ...
        BDComando.CommandText = "SELECT M.CVE_MAQUILADOR,M.ENCARGADO,ISNULL((Select MIN(OPA.FECHAINICIO) FROM OP_ASIGNACION OPA WHERE OPA.Empresa = 1 AND OPA.Cve_Maquilador = M.Cve_Maquilador AND OPA.Finalizada = 0 AND OPA.Cancelada = 0 AND OPA.Estatus = 'AUTORIZADA'),DateAdd(Month, -1, GETDATE())) AS FECHAMININICIO FROM Maquilador M WHERE M.STATUS = 1 AND M.TIPOTALLER IN ('MAQUILA', 'AMBOS') ORDER BY M.ENCARGADO"
