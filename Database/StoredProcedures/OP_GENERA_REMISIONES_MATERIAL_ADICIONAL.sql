...
/****** Object:  StoredProcedure [dbo].[OP_GENERA_REMISIONES_MATERIAL_ADICIONAL]    Script Date: 18/07/2025 12:03:08 p. m. ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[OP_GENERA_REMISIONES_MATERIAL_ADICIONAL]
--DECLARE
	@EMPRESA BIGINT,
	@NO_OP BIGINT,
...
--SET @COMPUTADORA = 'SISTEMASUNO'
	...
	BEGIN TRY
		...
		SELECT 
			...
			OPEM.Cantidad-(SELECT ISNULL(SUM(RM.Cantidad),0) FROM REMISION_MATERIAL RM WHERE RM.Empresa = OPA.Empresa AND RM.No_OP = OPA.No_OP AND RM.TipoMaterial = OPEM.TipoMaterial AND RM.Cve_Material = OPEM.Cve_Material AND RM.Estatus = 'AUTORIZADA')+(SELECT SUM(OPEMI.Cantidad) FROM OP_EXPLOSION_MATERIALES_INSPECTOR OPEMI WHERE OPEMI.Empresa = OPA.Empresa AND OPEMI.No_OP = OPA.No_OP AND OPEMI.TipoMovimiento = 'FALTANTE' AND OPEMI.TipoMaterial = OPEM.TipoMaterial AND OPEMI.Cve_Material = OPEM.Cve_Material),
			0
		FROM
			OP_ASIGNACION OPA,
		...
		AND OPEM.Cve_Material = @CVE_MATERIAL
		...
		AND OCAI.Cve_Material = @CVE_MATERIAL
		AND RM.No_OP = PIT.No_OP
		GROUP BY
			RM.TipoMaterial,
			RM.Cve_Material,
			RM.Origen_No,
		...
		--SELECT * FROM RESERVADO_MATERIAL WHERE No_OP = 2273 AND Cve_Material = '1971'
		--SELECT * FROM SUGERIDO_COMPRA WHERE No_OP = 2273 AND Cve_Material = '1971'
		...
		--ACTUALIZA EL ESTATUS DEL SUGERIDO DE COMPRA
		UPDATE
			SUGERIDO_COMPRA 
		SET
			STATUS = CASE WHEN Cantidad = 0 THEN 'REMISIONADO' ELSE 'PENDIENTE OP' END
		WHERE
			Empresa = @EMPRESA
		AND No_Pedido = @NO_PEDIDO
		AND TipoMaterial = @TipoMaterial
		AND Cve_Material = @CVE_MATERIAL
		AND Status IN ('INVENTARIO OP','PENDIENTE OP CON INVENTARIO')
		AND No_OP = @NO_OP
