...
/****** Object:  StoredProcedure [dbo].[OP_CONSULTA_TALLAS_CANTIDADES_INGRESO_TOTAL_RECIBIDO]    Script Date: 13/11/2025 07:23:38 p. m. ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[OP_CONSULTA_TALLAS_CANTIDADES_INGRESO_TOTAL_RECIBIDO]
--DECLARE
	@EMPRESA BIGINT,
	@NO_OP BIGINT
AS
...
--	SET @NO_OP = 1369
	...
	SELECT
		...
		(SELECT SUM(PEI.Cantidad) FROM PRENDA_ENTRADA_INVENTARIO PEI WHERE PEI.Empresa = @EMPRESA AND PEI.TipoEntrada = 'OP' AND PEI.TipoEntradaNumero = OPA.No_OP AND PEI.Talla = PIT.Talla AND PEI.Estatus = 'AUTORIZADO'),
		(SELECT SUM(PEI.Cantidad) FROM PRENDA_ENTRADA_INVENTARIO PEI WHERE PEI.Empresa = @EMPRESA AND PEI.TipoEntrada = 'OP' AND PEI.TipoEntradaNumero = OPA.No_OP AND PEI.Estatus = 'AUTORIZADO'),
		0
	FROM
		OP_ASIGNACION OPA,
		PEDIDO_INTERNO PI,
	...
	SET @query = '
	SELECT NO_OP, CVE_MAQUILADOR, NOM_MAQUILADOR, No_Pedido, Cve_Prenda, DescripcionPrenda, Cve_Cliente, Nom_Cliente, FechaEntrada,No_Entrada,' + '''Total Recibido''' + ' as FolioManual,' + @tallas + ',Total,ColumnaControl
	FROM
	(
...
--OP_CONSULTA_TALLAS_CANTIDADES_INGRESO_TOTAL_RECIBIDO 1,1369
GO
