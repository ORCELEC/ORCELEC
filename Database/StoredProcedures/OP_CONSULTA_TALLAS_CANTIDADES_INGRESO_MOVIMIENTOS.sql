...
/****** Object:  StoredProcedure [dbo].[OP_CONSULTA_TALLAS_CANTIDADES_INGRESO_MOVIMIENTOS]    Script Date: 13/11/2025 07:24:00 p. m. ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[OP_CONSULTA_TALLAS_CANTIDADES_INGRESO_MOVIMIENTOS]
--DECLARE
	@EMPRESA BIGINT,
	@NO_OP BIGINT
AS
...
--	SET @NO_OP = 1369
	...
	SELECT
		...
		PIT.Talla,
		SUM(PEI.Cantidad),
		(SELECT ISNULL(SUM(PEI1.Cantidad),0) FROM PRENDA_ENTRADA_INVENTARIO PEI1 WHERE PEI1.Empresa = @EMPRESA AND PEI1.TipoEntrada = 'OP' AND PEI1.TipoEntradaNumero = OPA.No_OP AND PEI1.No_Entrada = PEI.No_Entrada AND PEI1.Estatus = 'AUTORIZADO'),
		0
	FROM
		OP_ASIGNACION OPA,
		PEDIDO_INTERNO PI,
		...
		LEFT JOIN
			PRENDA_ENTRADA_INVENTARIO PEI
		ON
			PEI.Empresa = PIT.Empresa
		AND PEI.TipoEntrada = 'OP'
		AND PEI.TipoEntradaNumero = PIT.No_OP
		AND PEI.Talla = PIT.Talla
		AND PEI.Estatus = 'AUTORIZADO',
		TALLAS_GENERALES TG
	WHERE
		OPA.Empresa = @EMPRESA
	AND OPA.No_OP = @NO_OP
	...
	SET @query = '
	SELECT NO_OP, CVE_MAQUILADOR, NOM_MAQUILADOR, No_Pedido, Cve_Prenda, DescripcionPrenda, Cve_Cliente, Nom_Cliente, FechaEntrada,No_Entrada,FolioManual,' + @tallas + ',Total,ColumnaControl
	FROM
	(
		SELECT OP.NO_OP, OP.CVE_MAQUILADOR, OP.NOM_MAQUILADOR, OP.No_Pedido, OP.Cve_Prenda, OP.DescripcionPrenda, OP.Cve_Cliente, OP.Nom_Cliente,OP.FechaEntrada,OP.No_Entrada,OP.FolioManual,OP.Talla, OP.Cantidad,OP.Total,OP.ColumnaControl
		FROM #ORDEN_PRODUCCION OP WHERE NO_ENTRADA IS NOT NULL
	) AS SourceTable
	PIVOT
	(
		SUM(Cantidad)
