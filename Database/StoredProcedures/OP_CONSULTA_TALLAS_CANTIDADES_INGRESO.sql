...
/****** Object:  StoredProcedure [dbo].[OP_CONSULTA_TALLAS_CANTIDADES_INGRESO]    Script Date: 13/11/2025 07:23:24 p. m. ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[OP_CONSULTA_TALLAS_CANTIDADES_INGRESO]
--DECLARE
	@EMPRESA BIGINT,
	@NO_OP BIGINT
AS
...
--	SET @NO_OP = 1369
	...
	SELECT
		...
		(SELECT SUM(PIT1.Cantidad-ISNULL(RIPT1.Cantidad,0)) FROM PEDIDO_INTERNO_TALLAS PIT1 LEFT JOIN RESERVADO_INVENTARIO_PRODUCTO_TERMINADO RIPT1 ON RIPT1.Empresa = PIT1.Empresa AND RIPT1.No_Pedido = PIT1.No_Pedido AND RIPT1.Partida = PIT1.Partida AND RIPT1.Cve_Prenda = PIT1.Cve_Prenda AND RIPT1.LugarDeEntrega = PIT1.LugarDeEntrega AND RIPT1.Prioridad = PIT1.Prioridad AND RIPT1.Talla = PIT1.Talla WHERE PIT1.Empresa = @EMPRESA AND PIT1.No_OP = @NO_OP),
		0
	FROM
		OP_ASIGNACION OPA,
		PEDIDO_INTERNO PI,
...
--OP_CONSULTA_TALLAS_CANTIDADES_INGRESO 1,1369
GO
