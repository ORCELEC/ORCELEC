...
/****** Object:  StoredProcedure [dbo].[SP_PEDIDO_INTERNO_GUARDA_ASIGNACION_OP_LUGARENTREGA]    Script Date: 09/07/2025 02:30:13 p. m. ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SP_PEDIDO_INTERNO_GUARDA_ASIGNACION_OP_LUGARENTREGA] --OPTIMIZADOR POR LUGAR DE ENTREGA
--DECLARE
	@EMPRESA AS BIGINT,
	@NO_PEDIDO AS BIGINT,
...
BEGIN
	...
	--SET @NO_PEDIDO = 1176
	--SET @SUBTIPO = 0
	--INSERT INTO @Agrupacion VALUES(15,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(16,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(17,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(18,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(19,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(20,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(21,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(22,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(23,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(24,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(25,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(26,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(27,'CVE_PRENDA')
	--INSERT INTO @Agrupacion VALUES(28,'CVE_PRENDA')
	--SET @USUARIO = 0
	...
	BEGIN TRY
		...
		SET @TRAN_STARTED = 1
			...
			--****CODIGO ANTERIOR DE #OCPARCIALIDADES
			...
			DECLARE
				...
				@OCAUX NUMERIC(18,4),
				@PARTIDAOCAUX BIGINT,
				@CANTIDADPROMESAOCAUX NUMERIC(18,4),
				@FECHAPROMESAOCAUX DATE,
				@NO_PARCIALIDAD BIGINT,
				@CANTIDADPOSITIVAS INT,
				@CANTIDADNEGATIVAS INT,
				@SOBRANTE NUMERIC(18,4),
				@CONSECUTIVO_PRIORIDAD INT,
				@TIENE_TABLA_MEDIDA AS INT
			...
			(
				...
				CantidadFechaPromesa numeric(18,0),
				FechaPromesa date,
				AsignacionPromesa numeric(18,4),
				SaldoConsumo numeric(18,4),
				SobrantePromesa numeric(18,4)
			...
			ORDER BY
				...
				CVE_MATERIAL
			OPEN LISTAOP
			FETCH NEXT FROM LISTAOP INTO @NO_PEDIDO,@TIPOMATERIAL,@CVE_MATERIAL,@DESCRIPCIONMATERIAL
			WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				SET @OCAUX = 0
				SET @PARTIDAOCAUX = 0
				SET @CANTIDADPROMESAOCAUX = 0
				SET @FECHAPROMESAOCAUX = NULL
				...
				SET @CONSECUTIVO_PRIORIDAD = 1

				--ENUMERA LA LISTA DE PRIORIDADES EN ORDEN
				DECLARE PRIORIDADES CURSOR FOR
				SELECT
					PRIORIDAD
				FROM
					#PRIORIDADES
				ORDER BY
					PRIORIDAD
				OPEN PRIORIDADES
				FETCH NEXT FROM PRIORIDADES INTO @PRIORIDAD
				WHILE @@FETCH_STATUS = 0
				BEGIN
					UPDATE
						#PRIORIDADES
					SET
						CONSECUTIVO = @CONSECUTIVO_PRIORIDAD
					WHERE
						PRIORIDAD = @PRIORIDAD
				...
				SELECT
					TOP 1
					@CONSECUTIVO_PRIORIDAD = CONSECUTIVO
				FROM
					#PRIORIDADES
				ORDER BY
					CONSECUTIVO
		...
		INICIOCICLO:			
					...
					AND P.CONSECUTIVO = @CONSECUTIVO_PRIORIDAD
					AND XP.PRIORIDAD = P.PRIORIDAD
					...
					BEGIN
						...
						AND EMLE.PRIORIDAD = P.PRIORIDAD
						AND P.CONSECUTIVO = @CONSECUTIVO_PRIORIDAD
						...
						--COMO SE ASIGNO LA PRIORIDAD COMPLETA QUE ES NIVEL MÁS BAJO SE PASA A LA SIGUIENTE PRIORIDAD
						SET @CONSECUTIVO_PRIORIDAD += 1

						--SE VERIFICA QUE EXISTA LA SIGUIENTE PRIORIDAD SINO SE DEJA CORRER EL PROCESO
						IF EXISTS(SELECT * FROM #PRIORIDADES WHERE CONSECUTIVO = @CONSECUTIVO_PRIORIDAD)
							--SI EXISTE SE REINICIA EL PROCESO HASTA LA ÚLTIMA PRIORIDAD
							GOTO INICIOCICLO

					END
					---**********************TERMINA DE ASIGNAR LA PRIORIDAD CUANDO SE CUBRE COMPLETAMENTE********************************
					...
					BEGIN
						...
						BEGIN
							...
							FROM
								...
								#PRIORIDADES P
							WHERE
								A.NO_PEDIDO = (SELECT TOP 1 XLE.NO_PEDIDO FROM #XLUGARENTREGA XLE WHERE XLE.NO_PEDIDO = @NO_PEDIDO AND XLE.TIPOMATERIAL = @TIPOMATERIAL AND XLE.CVE_MATERIAL = @CVE_MATERIAL AND XLE.DESCRIPCIONMATERIAL = @DESCRIPCIONMATERIAL AND XLE.PRIORIDAD = P.PRIORIDAD AND XLE.SaldoConsumo > 0 AND XLE.SobrantePromesa >= 0 ORDER BY XLE.SobrantePromesa ASC)
						...
						BEGIN
							...
							VERIFICAPRIORIDAD:
								...
								--SE VERIFICA QUE LA PRIORIDAD EXISTA
								IF EXISTS(SELECT * FROM #PRIORIDADES WHERE CONSECUTIVO = @CONSECUTIVO_PRIORIDAD)
								BEGIN
									--SE VERIFICA QUE HAYA PENDIENTES EN ESA PRIORIDAD PARA ASIGNAR
									IF EXISTS(SELECT * FROM #PRIORIDADES P,#XPRIORIDAD XP WHERE P.CONSECUTIVO = @CONSECUTIVO_PRIORIDAD AND XP.PRIORIDAD = P.PRIORIDAD AND XP.SaldoConsumo > 0)
									BEGIN
										GOTO INICIOCICLO
									END
									ELSE
									BEGIN
										--SI YA NO HAY PENDIENTES DE ASIGNAR EN LA SIGUIENTE PRIORIDAD SE REINICIA EL CICLO
										GOTO VERIFICAPRIORIDAD
									END
								END
								ELSE
								BEGIN
									--QUIERE DECIR QUE ES LA ULTIMA PRIORIDAD, QUE YA INTENTO ASIGNAR EL MAYOR SALDO DE LA 
									--PARCIALIDAD ACTUAL, ENTONCES SE PASA CON LA SIGUIENTE PARCIALIDAD
								VERIFICARPARCIALIDAD:
									SET @NO_PARCIALIDAD += 1
									--SE VERIFICA QUE EXISTA LA PARCIALIDAD
									IF EXISTS(SELECT * FROM #OCPARCIALIDADES WHERE NO_PEDIDO = @NO_PEDIDO AND TIPOMATERIAL = @TIPOMATERIAL AND CVE_MATERIAL = @CVE_MATERIAL AND DESCRIPCION_MATERIAL = @DESCRIPCIONMATERIAL AND NO_PARCIALIDAD = @NO_PARCIALIDAD)
									BEGIN
										--SE VERIFICA QUE LA PARCIALIDAD SIGUIENTE TENGA SALDO
										IF EXISTS(SELECT * FROM #OCPARCIALIDADES WHERE NO_PEDIDO = @NO_PEDIDO AND TIPOMATERIAL = @TIPOMATERIAL AND CVE_MATERIAL = @CVE_MATERIAL AND DESCRIPCION_MATERIAL = @DESCRIPCIONMATERIAL AND NO_PARCIALIDAD = @NO_PARCIALIDAD AND CANTIDADPROMESA > 0)
										BEGIN
											--SE OBTIENE LA PRIORIDAD MENOR QUE TIENE SALDOCONSUMO POR ASIGNAR
											SELECT
											...
											--SE EMPIEZA DE NUEVO EL PROCESO
											--SELECT 'VA A INICIO',@CONSECUTIVO_PRIORIDAD,@NO_PARCIALIDAD
											GOTO INICIOCICLO
										END
										ELSE
										BEGIN
											--SI LA PARCIALIDAD NO TIENE SALDO,
											GOTO VERIFICARPARCIALIDAD
										END
									END
									ELSE
									BEGIN
										--ES LA ULTIMA PARCIALIDAD, ENTONCES SE COMPRUEBA QUE EFECTIVAMENTE NO HAYA MÁS ASIGNACIONES QUE SE PUEDAN
										--HACER POR LA TOTALIDAD DE LA PRENDA CON UNA SOLA PARCIALIDAD
										IF EXISTS(
											SELECT
												EMLE.*
											FROM
											...
											AND EMLE.CVE_MATERIAL = @CVE_MATERIAL
											AND EMLE.CONSUMOSALDO > 0
											AND OC.NO_PEDIDO = EMLE.NO_PEDIDO
											AND OC.TIPOMATERIAL = EMLE.TIPOMATERIAL
											AND OC.CVE_MATERIAL = EMLE.CVE_MATERIAL
											AND OC.CANTIDADPROMESA > EMLE.CONSUMOSALDO
										)
										BEGIN
											--QUIERE DECIR QUE HAY SALDOS QUE SI SE PUEDEN CUBRIR CON LA TOTALIDAD DE UNA PARCIALIDAD
									
											--SE OBTIENE LA PRIORIDAD DONDE SE PUEDE ASIGNAR Y LA PARCIALIDAD DE LA CUAL SE PUEDE ASIGNAR
											SELECT 
												TOP 1
												@PRIORIDAD = EMLE.PRIORIDAD,
												@NO_PARCIALIDAD = OC.NO_PARCIALIDAD
											FROM
												#OCPARCIALIDADES OC,
												#EMXOP EMLE
										...
										BEGIN
											...
											FETCH NEXT FROM FILASASIGNASALDOS INTO @PRIORIDAD,@LUGARENTREGA,@CVE_PRENDA
											WHILE @@FETCH_STATUS = 0
											BEGIN
												--SE ASIGNA EL PRIMER SALDO
											...
											CICLOASIGNASALDOS:
												UPDATE
													#XCVEPRENDA
												SET
													No_Parcialidad = (SELECT TOP 1 OC.NO_PARCIALIDAD FROM #OCPARCIALIDADES OC WHERE OC.NO_PEDIDO = XP.NO_PEDIDO AND OC.TIPOMATERIAL = XP.TIPOMATERIAL AND OC.CVE_MATERIAL = XP.CVE_MATERIAL AND OC.CANTIDADPROMESA > 0 ORDER BY OC.CANTIDADPROMESA DESC),
													No_Parcialidad_Original = (SELECT TOP 1 OC.NO_PARCIALIDAD_ORIGINAL FROM #OCPARCIALIDADES OC WHERE OC.NO_PEDIDO = XP.NO_PEDIDO AND OC.TIPOMATERIAL = XP.TIPOMATERIAL AND OC.CVE_MATERIAL = XP.CVE_MATERIAL AND OC.CANTIDADPROMESA > 0 ORDER BY OC.CANTIDADPROMESA DESC),
													OCPromesa = (SELECT TOP 1 OC.NO_ORDENCOMPRA FROM #OCPARCIALIDADES OC WHERE OC.NO_PEDIDO = XP.NO_PEDIDO AND OC.TIPOMATERIAL = XP.TIPOMATERIAL AND OC.CVE_MATERIAL = XP.CVE_MATERIAL AND OC.CANTIDADPROMESA > 0 ORDER BY OC.CANTIDADPROMESA DESC),
													PartidaPromesa = (SELECT TOP 1 OC.PARTIDA FROM #OCPARCIALIDADES OC WHERE OC.NO_PEDIDO = XP.NO_PEDIDO AND OC.TIPOMATERIAL = XP.TIPOMATERIAL AND OC.CVE_MATERIAL = XP.CVE_MATERIAL AND OC.CANTIDADPROMESA > 0 ORDER BY OC.CANTIDADPROMESA DESC),
													CantidadFechaPromesa = (SELECT TOP 1 OC.CANTIDADPROMESA FROM #OCPARCIALIDADES OC WHERE OC.NO_PEDIDO = XP.NO_PEDIDO AND OC.TIPOMATERIAL = XP.TIPOMATERIAL AND OC.CVE_MATERIAL = XP.CVE_MATERIAL AND OC.CANTIDADPROMESA > 0 ORDER BY OC.CANTIDADPROMESA DESC),
													FechaPromesa = (SELECT TOP 1 OC.FECHAPROMESA FROM #OCPARCIALIDADES OC WHERE OC.NO_PEDIDO = XP.NO_PEDIDO AND OC.TIPOMATERIAL = XP.TIPOMATERIAL AND OC.CVE_MATERIAL = XP.CVE_MATERIAL AND OC.CANTIDADPROMESA > 0 ORDER BY OC.CANTIDADPROMESA DESC),
													SobrantePromesa = (SELECT TOP 1 OC.CANTIDADPROMESA-XP.SaldoConsumo FROM #OCPARCIALIDADES OC WHERE OC.NO_PEDIDO = XP.NO_PEDIDO AND OC.TIPOMATERIAL = XP.TIPOMATERIAL AND OC.CVE_MATERIAL = XP.CVE_MATERIAL AND OC.CANTIDADPROMESA > 0 ORDER BY OC.CANTIDADPROMESA DESC)
												FROM
													#XCVEPRENDA XP
												WHERE
											...
											--SELECT * FROM #EMXOP WHERE NO_PEDIDO = @NO_PEDIDO AND TIPOMATERIAL = @TIPOMATERIAL AND CVE_MATERIAL = @CVE_MATERIAL AND CONSUMOSALDO > 0
										END
									END
								END
							END
			...
			---**************************************************************************************************************************************
			--*******************TERMINA CODIGO NUEVO INSERTADO EL 10/11/2022
			...
			CREATE TABLE #AUXQUERY_LUGARENTREGA
			(
				NO_PEDIDO NUMERIC(18,0),
				PRIORIDAD BIGINT,
				LUGARENTREGA BIGINT,
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN--GUARDA EL REGISTRO PREVIO
						...
						INSERT INTO #AUXQUERY_LUGARENTREGA
						(
							NO_PEDIDO,
							PRIORIDAD,
							LUGARENTREGA,
			...
			UPDATE
				#NO_OP_ASIGNACION
			SET
				FECHAASIGNACION = (SELECT MAX(EMLE.FECHAASIGNACION) FROM #EMXOP EMLE WHERE #NO_OP_ASIGNACION.NO_PEDIDO = EMLE.NO_PEDIDO AND #NO_OP_ASIGNACION.PRIORIDAD = EMLE.PRIORIDAD AND #NO_OP_ASIGNACION.LUGARENTREGA = EMLE.LUGARENTREGA AND #NO_OP_ASIGNACION.CVE_PRENDA = EMLE.CVE_PRENDA AND #NO_OP_ASIGNACION.DESCRIPCIONPRENDA = EMLE.DESCRIPCIONPRENDA),
				LOTES = (SELECT MAX(EMLE.LOTES) FROM #EMXOP EMLE WHERE #NO_OP_ASIGNACION.NO_PEDIDO = EMLE.NO_PEDIDO AND #NO_OP_ASIGNACION.PRIORIDAD = EMLE.PRIORIDAD AND #NO_OP_ASIGNACION.LUGARENTREGA = EMLE.LUGARENTREGA AND #NO_OP_ASIGNACION.CVE_PRENDA = EMLE.CVE_PRENDA AND #NO_OP_ASIGNACION.DESCRIPCIONPRENDA = EMLE.DESCRIPCIONPRENDA AND EMLE.TIPOMATERIAL = 'T')
			--SELECT * FROM #NO_OP_ASIGNACION
			CREATE TABLE #OP_FINAL_LUGARENTREGA
			(
				NO_OP BIGINT,
				NO_PEDIDO BIGINT,
				CVE_PRENDA BIGINT,
			...
			select * from #NO_OP_ASIGNACION
			INSERT INTO #OP_FINAL_LUGARENTREGA
			(
				NO_OP,
				NO_PEDIDO,
				CVE_PRENDA,
			...
			UPDATE
				#OP_FINAL_LUGARENTREGA
			SET
				FECHAASIGNACION = (SELECT MAX(EMLE.FECHAASIGNACION) FROM #NO_OP_ASIGNACION EMLE WHERE #OP_FINAL_LUGARENTREGA.NO_PEDIDO = EMLE.NO_PEDIDO AND #OP_FINAL_LUGARENTREGA.NO_OP = EMLE.NO_OP),
				LOTES = (SELECT MAX(EMLE.LOTES) FROM #NO_OP_ASIGNACION EMLE WHERE #OP_FINAL_LUGARENTREGA.NO_PEDIDO = EMLE.NO_PEDIDO AND #OP_FINAL_LUGARENTREGA.NO_OP = EMLE.NO_OP)

			CREATE TABLE #OP_FINAL_LUGARENTREGA_AGRUPACION
			(
			...
			BEGIN
				...
				DBCC CHECKIDENT (#OP_FINAL_LUGARENTREGA_AGRUPACION, RESEED, @NO_OP)
				...
				BEGIN
					...
					BEGIN
						INSERT INTO #OP_FINAL_LUGARENTREGA_AGRUPACION
						(
							NO_PEDIDO,
							CVE_PRENDA,
							DESCRIPCIONPRENDA,
						...
						FROM 
							#OP_FINAL_LUGARENTREGA OPFLE,
							#OP_FINAL_LUGARENTREGA_AGRUPACION OPFLEA
						WHERE
							OPFLE.CVE_PRENDA = @CVE_PRENDA
					...
					BEGIN
						...
						SELECT
							...
							OPFLE.DESCRIPCIONPRENDA,
							SUM(OPFLE.CANTIDAD),
							(SELECT MAX(OPFLE1.FECHAASIGNACION) FROM #OP_FINAL_LUGARENTREGA OPFLE1 WHERE OPFLE1.NO_PEDIDO = OPFLE.NO_PEDIDO AND OPFLE1.CVE_PRENDA = OPFLE.CVE_PRENDA),
							(SELECT MAX(OPFLE1.LOTES) FROM #OP_FINAL_LUGARENTREGA OPFLE1 WHERE OPFLE1.NO_PEDIDO = OPFLE.NO_PEDIDO AND OPFLE1.CVE_PRENDA = OPFLE.CVE_PRENDA)
						FROM 
							#OP_FINAL_LUGARENTREGA OPFLE
						WHERE
							OPFLE.CVE_PRENDA = @CVE_PRENDA
			...
			--SELECT * FROM #OP_FINAL_LUGARENTREGA
			--SELECT * FROM #OP_FINAL_LUGARENTREGA_AGRUPACION

		--**********************************TERMINA PROCESO PARA LUGAR DE ENTREGA*************************************************************
			...
			BEGIN
				...
				ORDER BY
					OP.NO_OP
				OPEN ListaOP
				FETCH NEXT FROM ListaOP INTO @CVE_PRENDA,@NO_OP
				WHILE @@FETCH_STATUS = 0
				BEGIN
					IF EXISTS(SELECT
						*
					FROM
						PEDIDO_INTERNO_TALLAS PIT,
						PRENDA_TABLA_MEDIDA TM
					WHERE
						PIT.Empresa = @EMPRESA
					AND	PIT.No_OP = @NO_OP
					AND TM.Cve_Prenda = PIT.Cve_Prenda
					)
					BEGIN
						INSERT INTO OP_PROCESOS
						(
							Empresa,
							No_OP,
							Cve_Prenda,
			...
			BEGIN
				...
				BEGIN
					...
					SELECT
						...
						0
					FROM
						#OP_FINAL_LUGARENTREGA_AGRUPACION OP
			...
			SELECT
				...
				PEMD.Descripcion,
				--CASE WHEN TipoMaterial = 'T' THEN SUM((PIT.CANTIDAD*PEMD.Consumo)/CT.ANCHO) ELSE SUM(PIT.CANTIDAD*PEMD.Consumo) END,
				CASE WHEN TipoMaterial = 'T' THEN SUM(((PIT.CANTIDAD-ISNULL(RIPT.CANTIDAD,0))*PEMD.Consumo)/CT.ANCHO) ELSE SUM((PIT.CANTIDAD-ISNULL(RIPT.CANTIDAD,0))*PEMD.Consumo) END,
				0,
				0,
				0,
				GETDATE()
			...
			DROP TABLE #AUXQUERY_LUGARENTREGA
			DROP TABLE #AUX
			DROP TABLE #AUXGLOBAL
			DROP TABLE #OP_FINAL_LUGARENTREGA
			DROP TABLE #NO_OP_ASIGNACION
			DROP TABLE #OP_FINAL_LUGARENTREGA_AGRUPACION
