...
Public Class AutorizaPedido
    ...
    Private Sub AutorizaPedido_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub ConsultaPedidosAAutorizar()
        ...
        BDComando.CommandText = "SELECT * FROM PEDIDO_INTERNO WHERE STATUS = 'A AUTORIZAR'"
        Try
            ...
            If ListPedidos.SelectedItems.Count > 0 Then
                If MessageBox.Show("¿Esta seguro de Autorizar el pedido interno No. " & ListPedidos.SelectedItem.ToString() & "?", "Autorización de pedido interno", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                    ...
                    Try
                        ...
                        ' Separar y agregar todos los destinatarios
                        ...
                        builder.HtmlBody = "<html><body><h2>Se explosionó el pedido interno No. " & Val(ListPedidos.SelectedItem.ToString()) & _
                                           " y se generó sugerido de compra</h2><h2>Cliente: " & TxtCliente.Text & "</h2>" & CuerpoCorreo & "</body></html>"
                        ...
                        'correo.From = New MailAddress(ConectaBD.MailUsuario, "ORCELEC")
                        'correo.Subject = "Autorización de Pedido No. " & ListPedidos.SelectedItem.ToString()
                        'If Email <> "" Then
                        '    correo.To.Add("ch@uet.mx,amm@uet.mx,dpa@uet.mx,lmc@uet.mx,mlg@uet.mx,apm@uet.mx" & "," & Email)
                        'Else
                        '    correo.To.Add("ch@uet.mx,amm@uet.mx,dpa@uet.mx,lmc@uet.mx,mlg@uet.mx,apm@uet.mx")
                        'End If
                        ''correo.To.Add("ch@uet.mx")
                        'correo.Body = "<html><body><h2>Se explosionó el pedido interno No. " & Val(ListPedidos.SelectedItem.ToString()) & " y se generó sugerido de compra</h2><h2>Cliente: " & TxtCliente.Text & "</h2>" & CuerpoCorreo & "</body></html>"
                        ...
                        'Servidor.Credentials = New NetworkCredential(ConectaBD.MailUsuario, ConectaBD.PasswordCorreo)
                        'AddHandler Servidor.SendCompleted, AddressOf SendCompletedCallback
                        'Servidor.SendAsync(correo, "AutorizaPedido")
                        'Servidor.Send(correo) 'Se envía correo de notificación.
                    Catch ex As Exception
            ...
            If ListPedidos.SelectedItems.Count > 0 Then
                If MessageBox.Show("¿Esta seguro de querer cancelar el pedido interno No. " & ListPedidos.SelectedItem.ToString() & "?", "Autorización de pedido interno", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                    ...
                    Try
                        ...
                        ReiniciaControles()
                    Catch ex As Exception
                        MessageBox.Show("Se generó un error al cancelar el pedido, favor de contactar a sistemas y dar como referencia el siguiente mensaje." & vbCrLf & "-" & ex.Message, "Autorización de Pedido", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                        Exit Sub
                    Finally
                        ' Asegurarse de que el DataReader y la conexión se cierren.
                        If Not BDReader Is Nothing AndAlso Not BDReader.IsClosed Then
    ...
    Private Sub BtnMuestraLugarCobro_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    '                    Next
    '                    DGTallasCantPrecios.Columns("TotalPartida").HeaderText = "Total de la Partida"
    '                    DGTallasCantPrecios.Columns("TotalPartida").Width = 70
